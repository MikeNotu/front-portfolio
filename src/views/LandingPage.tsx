import "../styles/LandingPage.scss";
import Img_Man from "../media/Man.png";
import Img_Mike from "../media/Mike.png";

const LandingPage = () => {
  return (
    <div>
      {/* <div className="sec-1"></div> */}

      <div className="landing-page">
        <div className="bubbles">
          <div className="sec-1">
            <div className="technologies">🧐testtest</div>
            <div className="jumbotron">
              <img className="man" src={Img_Man} alt="Man Image" />
              <div className="jumbotron__description">
                <h1>Michael Bateman</h1>
                <h4 className="jumbotron__description--sec3">{`React & React Native Developer`}</h4>
                <p className="jumbotron__description--sec4">
                  I'm a Software Engineering, specialized in FrontEnd
                  Development.
                </p>
                <p className="jumbotron__description--sec4">
                  I'm always happy and willing to learn any technology that's
                  needed for a project.
                </p>
              </div>
            </div>
          </div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
          <div className="bubble"></div>
        </div>
      </div>
    </div>
  );
};

export default LandingPage;
